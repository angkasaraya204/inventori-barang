        ...
        <div class="w-full flex justify-center">
            <div class="bg-white p-6 rounded-lg mx-auto w-96 md:w-1/4 absolute top-20">
                <div class="text-center">
                    <img src="https://i.ibb.co.com/6bzVPKG/2-KA03-SI-Black.png" alt="logo"
                ...
                <div class="mt-10">
                    ...
                    <form action="{{route('login')}}" method="post">
                        @csrf
                        <label class="text-sm text-slate-400 font-semibold uppercase mt-5 inline-block" for="email">
                            Email
                        </label>
                        <div class="border @error('email') border-red-500 @enderror mt-2 p-2">
                            <input id="email" class="w-full h-full text-sm focus:outline-none" type="email" name="email" value="{{old('email')}}" placeholder="alamat email" />
                        </div>
                        @error('email') 
                          <p class="italic mt-1 text-red-500 text-xs">{{$message}}</p> 
                        @enderror
                        <div class="flex justify-between mt-5">
                            <label class="text-sm text-slate-400 font-semibold uppercase" for="password">
                                Kata Sandi
                            </label>
                        </div>
                        <div class="border mt-2 p-2 @error('password') border-red-500 @enderror">
                            <input id="password" class="w-full h-full text-sm focus:outline-none" type="password"
                                name="password" placeholder="kata sandi" />
                        </div>
                        @error('password') 
                          <p class="italic text-red-500 text-xs">{{$message}}</p> 
                        @enderror
                        <button class="bg-blue-700 text-white text-center w-full mt-5 rounded-lg py-2 text-sm">
                            Masuk
                        </button>
                    </form>
                </div>
